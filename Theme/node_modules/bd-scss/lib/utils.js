...
export const getConfig = async () => {
    const local = (await import(path.join(process.cwd(), `bd-scss.config.js`))).default;
    const CONFIG = {
        meta: local.meta,
 ...
 */
export const getDataFolder = (mod) => {
    ...
    if (process.platform === 'win32') {
        ...
        throw new Error('Cannot determin your OS.');
    if (folder[0] === '~')
        folder = process.env.HOME + folder.substring(1);
    return folder;
};
/**
 * Transforms the given value to an absolute path.
 */
export const getPath = (...val) => {
 ...
 */
export const pop = (str) => str.split('/').pop().replace(/^_/, '');
/**
 * Generate a CSS import based on the `bd-scss.config.js` file.
 * @param config The config object.
 * @param name The name of the file.
 */
export const generateImport = (config, name) => {
