...
const compileSCSS = (file, silent = false) => {
    if (!fs.existsSync(file)) {
        log.error(`Cannot find the file: ${log.code(file)}`);
    }
    if (!silent)
        log.info(`Building ${log.code(file)}...`);
...
const writeToDisk = (name, file, dir) => {
    const dirPath = dir
        .split(osSlash)
...
export const compileDev = async () => {
    ...
    if (config.vencord.dev)
        writeToDisk(name, result, config.vencord.themeFolder);
